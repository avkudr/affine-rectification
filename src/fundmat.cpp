...
//        AR_Printf("Cannot initialize the fundamental matrix");
//        return;
//    }
    _pts1 = pts1;
    _pts2 = pts2;
...
cv::Mat FundMat::findFundMatGSransac(const std::vector<cv::Point2d> &_pts1, const std::vector<cv::Point2d> &_pts2)
{
    ...
    AR_Printf("RANSAC fraction : " << rans->getInliersFraction()*100 << "%");
    AR_Printf("Residual error : " << getResidualError(rans->getInliersIndices())
              << " (" << getResidualError() <<")");
...
cv::Mat FundMat::findFundMatGSlmeds(const std::vector<cv::Point2d> &_pts1, const std::vector<cv::Point2d> &_pts2)
{
    ...
    rans->solve(findFundMat);
    AR_Printf("LMedS fraction : " << rans->getInliersFraction()*100 << "%");
    AR_Printf("Residual error : " << getResidualError(rans->getInliersIndices())
              << " (" << getResidualError() <<")");
    return findFundMat->getResult();
}
